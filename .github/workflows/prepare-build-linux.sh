...
mkdir build
cd build
cmake -DMARCH=x86-64 -DPYTHON_EXECUTABLE=/opt/python/cp39-cp39/bin/python ..		# the gh-actions vm crashes with march=native,sandybridge or haswell, so use x86-64 just for the tests
...
rm -rf *
cmake -DPYTHON_EXECUTABLE=/opt/python/cp39-cp39/bin/python ..						# then compile again with the default march
make install -j 2